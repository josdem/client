...
public class DragTooltipDialog extends JDialog {
	...
	public enum IconType {
		Folder("folders", "dragFolder", Folder.class, 14, 14), Playlist("playlists", "dragPlaylist", Playlist.class, 14, 14), Track(
				"tracks", "dragTrack", Track.class, 14, 14), Download("downloads", "dragTrack", Download.class, 14, 14), Contact(
				"contacts", "dragContacts", ContactInfo.class, 7, 14) {
			@Override
			public boolean matches(Object arg0) {
				return super.matches(arg0) && ((ContactInfo) arg0).getStatus() != ContactStatus.pending;
			}
		},
		PendingEmail("pending emails", "dragPendingEmail", ContactInfo.class, 16, 14) {
			@Override
			public boolean matches(Object arg0) {
				return super.matches(arg0) && ((ContactInfo) arg0).getStatus() == ContactStatus.pending;
			}
	...
	public void setContent(DownloadCollection downloadCollection) {
		jContentPane.setName(DRAG_DIALOG_NAME);
		Dimension contentSize = fillContent(downloadCollection.getDownloads());
		this.setFullSize(contentSize);
	}

	public void setContent(ModelCollection modelCollection) {
		jContentPane.setName(DRAG_DIALOG_NAME);
		Dimension contentSize = fillContent(modelCollection.getFolders(), modelCollection.getPlaylists(),
				modelCollection.getTracks());
		this.setFullSize(contentSize);
	}
	...
	public void setContent(FileSystemValidatorLight validator) {
		...
		for (IconType type : IconType.values()) {
			...
			switch (type) {
			case Folder:
				list = validator.getFolders();
				break;
			case Playlist:
				list = validator.getPlaylists();
				break;
			case Track:
				list = validator.getTracks();
				break;
			}
			DynamicPanel dynamicPanel = getDynamicPanel(type, list);
	...
	public void setContent(ContactCollection contactCollection) {
		jContentPane.setName("dragContactDialog");
		Dimension contentSize = fillContent(contactCollection.getPendingContacts(), contactCollection.getContacts());
		this.setFullSize(contentSize);
	}
